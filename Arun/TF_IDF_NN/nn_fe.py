...
def init(embeddedfile="embedded.p", binarylabelsfile="binarylabels.p"):
    ...
    model.add(layers.Dense(500, name="last_hidden", activation="relu"))
...
def feature_extract(model, data):
    ...
    for i in range(20):
        ...
        features = extractor(data[s:e].toarray())
        with open("features_tfidf"+str(i)+".p", "wb") as f:
            pickle.dump(features, f)

if __name__ == "__main__":
    data, label, model = init(embeddedfile="embedded_20.p", binarylabelsfile="binarylabels_20.p")
    ...
    # save some outputs
    savecsv("true_1000_500_50.csv", label_test)
    savecsv("pred_1000_500_50.csv", pred)
    model.save("model_1000_500_50.h5")
