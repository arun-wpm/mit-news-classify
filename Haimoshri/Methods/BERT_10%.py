...
model = MyBertModel(HIDDEN_SIZE, NUM_LABEL).to(device)
...
for epoch in range(epochs):
    ...
    for inputs, masks, labels in train_data_loader:
        ...
        output = model.forward(inputs, masks.to(device))
...
for inputs, masks, labels in test_data_loader:
    ...
    inputs = inputs.type(torch.LongTensor).to(device)
    logits = model.forward(inputs, masks.to(device))
    ...
    accuracy += float(torch.sum(predictions == labels.to(device)))/593

accuracy = accuracy/count

savecsv("pred_BERT_10%.csv", all_pred)
savecsv("true_BERT_10%.csv", all_true)
